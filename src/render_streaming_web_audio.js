var render_streaming_web_audio = function() {
  ...
  function setup_onaudioprocess_callback_stream(
    given_node,
    cb_populate_memory_chunk,
    given_buff_size,
    given_num_channels
  ) {
    ...
    given_node.onaudioprocess = (function() {
      return function(event) {
        ...
        if (
          stop_next_event_loop_iteration ||
          queue_first_in_first_out.get_request_stop()
        ) {
        ...
        for (
          var curr_channel = 0;
          curr_channel < given_num_channels;
          curr_channel++
        ) {
          internal_audio_buffer_obj[
            curr_channel
          ] = event.outputBuffer.getChannelData(curr_channel);
        }
    ...
    return {
      ...
      get_value: function(given_property) {
        ...
        if (headers_obj && typeof headers_obj[given_property] !== "undefined") {
          ...
          throw new Error(
            "ERROR - failed to find headers property : " + given_property
          );
        }
      }
    };
  ...
  var queue_first_in_first_out = (function() {
    ...
    return {
      ...
      push: function(given_audio_obj_from_server) {
        ...
        while (size_buffer_available > offset_index) {
          // carve out render sized buffers from given buffer

          // stens TODO - put this into ww not here in browser land

          var array_buffer = new ArrayBuffer(
            BUFF_SIZE_AUDIO_RENDERER * Float32Array.BYTES_PER_ELEMENT
          );
          var float_array = new Float32Array(array_buffer);

          for (var i = 0; i < BUFF_SIZE_AUDIO_RENDERER; i++) {
            float_array.buffer[i] =
              given_audio_obj_from_server.buffer[i + offset_index];
          }
      ...
      is_pop_possible: function() {
        ...
        if (
          !flag_index_is_rising &&
          curr_size_queue <= browser_queue_min_threshold
        ) {
      ...
      pop: function() {
        ...
        if (pop_index < push_index) {
          ...
          throw new Error(
            "ERROR - boo hoo queue_first_in_first_out is EMPTY so cannot do a pop"
          );
        }
      },
    ...
    return function(given_audio_obj, num_channels) {
      ...
      if (queue_first_in_first_out.is_pop_possible()) {
        ...
        if (typeof audio_obj_from_queue === "undefined") {
          throw new Error(
            "ERROR - in get_another_buffer seeing undefined audio_obj_from_queue"
          );
        }
  ...
  function process_audio_buffer() {
    // only called upon initially retrieving audio fm svr

    if (
      (!queue_first_in_first_out.get_flag_audio_rendering() &&
      ...
      streaming_node = audio_context.createScriptProcessor(
      ...
      console.log(
        "OOOOOOOOOOOOOOOONNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN activating gain_node"
      );
      ...
      console.log(
        "OK just set flag_streaming_status    streaming_status_active"
      );
  ...
  function cb_send_buffer_to_web_audio_player(
    given_audio_obj,
    flag_early_or_not
  ) {
