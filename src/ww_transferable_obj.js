...
function cb_receive_buffer_from_server(given_audio_obj_from_server) {
  ...
  switch (curr_mode) {
    ...
    default: {
      throw new Error(
        "ERROR - invalid cb for buffer from server : " + curr_mode
      );
    }
  }
}
...
function cb_stream_is_complete(given_max_index) {
  send_console_to_browser.log(
    "cb_stream_is_complete ... given_max_index " + given_max_index
  );
...
var queue_first_in_first_out = (function() {
  ...
  return {
    ...
    set_max_size_ww_queue: function(given_max_size_ww_queue) {
      ...
      console.log(
        "set_max_size_ww_queue    max_size_ww_queue  ",
        max_size_ww_queue
      );
    },
...
var setup_stream_audio_from_server = function(msgs_to_server) {
  ...
  queue_first_in_first_out.set_max_size_ww_queue(
    msgs_to_server.mode_stream_audio_to_client.ww_queue_max_size
  );
...
self.onmessage = function(event) {
  //    retrieved a message from browser
  ...
  if (typeof event.data === "string") {
    ...
    if (typeof received_json.browser_directed_mode !== "undefined") {
      ...
      switch (
        received_json.browser_directed_mode // mode 3
      ) {
        case "browser_get_audio_from_ww": {
          manage_buffer_processing.set_mode(
        ...
        case "browser_get_audio_from_server": {
          ...
          specific_mode_stream.requested_source =
            received_json.requested_source;
        ...
        default: {
          send_console_to_browser.log(
            "ERROR - invalid browser_directed_mode : " +
              received_json.browser_directed_mode
          );
        }
      }
      ...
      send_console_to_browser.log(
        "ERROR - ww did receive string from browser" +
          "... yet NOT seeing browser_directed_mode"
      );
    }
  } else {
    send_console_to_browser.log(
      "ERROR - ww received NON string from browser" +
