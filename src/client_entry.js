var client_entry = (function() {
  ...
  var manage_state = (function() {
    ...
    return {
      ...
      set_browser_queue_filled: function() {
        ...
        if (
          ...
          if (manage_audio.get_is_streaming_done()) {
            console.log(
              "already reached end of source media AND ww queue should be EMPTY " +
          ...
          throw new Error(
            "ERROR - was told browser_queue_filled yet " +
              "current state not (1) or (3) : " +
              current_browser_mode
          );
        }
      },
      ...
      request_another_buffer: function() {
        ...
        if (current_browser_mode !== mode_ww_get_audio_from_server) {
          var returned_msg = manage_state.get_msg_to_server_by_mode(
            manage_state.get_state()
          );
        ...
        for (var curr_mode in arr_other_modes) {
          if (arr_other_modes.hasOwnProperty(curr_mode)) {
            for (var curr_property in given_msg) {
              msgs_to_server_by_mode[arr_other_modes[curr_mode]][
                curr_property
              ] = given_msg[curr_property];
            }
          }
        }

        msgs_to_server_by_mode[
          mode_ww_get_audio_from_server
        ].browser_directed_mode = mode_ww_get_audio_from_server;
        msgs_to_server_by_mode[
          mode_browser_get_audio_from_ww
        ].browser_directed_mode = mode_browser_get_audio_from_ww;
      },
  ...
  function browser_queue_is_full_callback() {
    console.log(
      "AAAAAAAAAAAAAAAAbout to call manage_state.set_browser_queue_filled"
    );
  ...
  var cb_request_another_buffer = (function() {
    // stens TODO - remove this as it just limits calls for ts
    ...
    return function(given_source) {
      if (max_requests === 0 || count_requests < max_requests) {
        ...
        if (
          ((given_source === "early_days" || given_source === "Middleburg") &&
            manage_audio.get_is_production_possible()) ||
        ...
        throw new Error(
          "ERROR - NOTICE - hold your horses ... intentionally stopping"
        );
      }
  ...
  var manage_audio = (function() {
    ...
    callback_send_audio_to_audio_player =
      web_audio_obj.cb_send_buffer_to_web_audio_player;
    ...
    web_audio_obj.queue_first_in_first_out.set_cb_browser_queue_is_full(
      browser_queue_is_full_callback
    );
    web_audio_obj.queue_first_in_first_out.set_cb_browser_queue_min_reached(
    ...
    return {
      ...
      set_queue_min_threshold: function(given_min_threshold) {
        web_audio_obj.queue_first_in_first_out.set_browser_queue_min_threshold(
          given_min_threshold
        );
      },
      set_queue_max_size: function(given_max_size) {
        web_audio_obj.queue_first_in_first_out.set_browser_queue_max_size(
          given_max_size
        );
      },
  ...
  function populate_launch_stream_audio_msg(msgs_to_server) {
    ...
    var array_render_N_chunk = {
      BUFF_SIZE_AUDIO_RENDERER: 16384,
      transmit_chunk_multiplier: 2
    };
    ...
    browser_queue_min_threshold =
      array_min_thr_N_size.browser_queue_min_threshold;

    stream_audio_msg.transmit_chunksize =
      BUFF_SIZE_AUDIO_RENDERER * transmit_chunk_multiplier;
      ...
      case "streaming_is_done": {
        ...
        if (typeof max_index !== "undefined") {
          ...
          if (!manage_audio.get_flag_audio_rendering()) {
            console.log(
              "flag    manage_audio.get_flag_audio_rendering " +
          ...
          throw new Error(
            "ERROR - seeing streaming_is_done yet not seeing max_index"
          );
        }
  ...
  var send_audio_to_server = (function() {
    ...
    return {
      ...
      output_media: function() {
        ...
        console.log(
          "array_retrieved_media_buffers length " + array_this_media.length
        );
  ...
  ww_handle.onmessage = function(event) {
    // handle traffic from ww
    ...
    if (event.data instanceof ArrayBuffer) {
      ...
      var rate_KB = Math.round(
        common_utils.toKB(data_from_ww.byteLength) / elapsed
      );
      ...
      callback_send_audio_to_audio_player(
        retrieved_audio_buffer_obj,
        manage_state.is_early_days()
      );
  ...
  var media_manager = (function() {
    ...
    all_media[5] =
      "Lee_Smolin_Physics_Envy_and_Economic_Theory-cWn86ESze6M_mono_1st_few_seconds_trim.wav";
    ...
    all_media[17] =
      "Justice_Genesis_chewy_chocolate_cookies_gtZunGHG0ls_mono.wav";
    ...
    return {
      get_media_filename: function(chosen_media) {
        ...
        if (typeof chosen_media_filename === "undefined") {
          throw new Error(
            "ERROR - failed to resolve media filename for chosen_media : " +
              chosen_media
          );
        }
  ...
  var console = (function() {
    ...
    function getScriptName() {
      ...
      var currentStackFrameRegex = new RegExp(
        /getScriptName \(.+\/(.*):\d+:\d+\)/
      );
    ...
    return {
      log: function(given_str) {
        if (
          typeof given_str === "string" &&
          given_str.indexOf("worker_log") > -1
        ) {
          common_utils.log(given_str);
        } else {
          common_utils.log(
            getScriptName() + " " + common_utils.source() + given_str
          );
        }
      }
    };
  ...
  var entry_point = (function() {
    //      handle traffic from browser UI
    ...
    manage_audio.set_BUFF_SIZE_AUDIO_RENDERER(
      msg_to_ww.mode_stream_audio_to_client.BUFF_SIZE_AUDIO_RENDERER
    );
    manage_audio.set_queue_min_threshold(
      msg_to_ww.mode_stream_audio_to_client.browser_queue_min_threshold
    );
    manage_audio.set_queue_max_size(
      msg_to_ww.mode_stream_audio_to_client.browser_queue_max_size
    );
    ...
    return function(request, chosen_media) {
      switch (request) {
        case "launch_stream_audio": {
          ...
          console.log("\n\n");
          console.log(
            "curr_msg_stream.browser_directed_mode " +
              curr_msg_stream.browser_directed_mode
          );
          console.log("                     media_file " + media_file);
          console.log("\n\n");
          ...
          manage_state.set_msg_to_server_by_mode(
            manage_state.get_state(),
            curr_msg_stream
          );
        ...
        case "stop_audio": {
          shared_utils.show_object(
            curr_msg_stop,
            "curr_msg_stop pp pp pp pp pp p",
            "total",
            10
          );
        ...
        default: {
          throw new Error(
            "ERROR - invalid request sent to entry_point : ",
            request
          );
        }
      }
