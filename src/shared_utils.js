var shared_utils = (function() {
  ...
  var release_all_prop_from_object = function(given_object) {
    // purge memory for object reuse
  ...
  var convert_32_bit_float_into_unsigned_16_bit_int_lossy = function(
    input_32_bit_buffer
  ) {
  ...
  var convert_16_bit_unsigned_int_to_32_bit_float = function(
    input_16_bit_int_buffer
  ) {
  ...
  var conv_bit_size = function(input_unsigned_int, bits_per_sample) {
    // converts unsigned into signed

    var max = (1 << (bits_per_sample - 1)) - 1;

    return input_unsigned_int > max
      ? input_unsigned_int - ((max << 1) + 2)
      : input_unsigned_int;
  };

  var convert_16_bit_signed_int_to_32_bit_float = function(
    input_8_bit_int_buffer
  ) {
    ...
    for (
      var index = 0;
      index < size_source_buffer;
      index += 2, index_32_bit_floats++
    ) {
      // tmp_16_bit_signed_int = (input_8_bit_int_buffer[index + 1] << 8) | input_8_bit_int_buffer[index];
      tmp_16_bit_unsigned_int =
        (input_8_bit_int_buffer[index + 1] << 8) |
        input_8_bit_int_buffer[index];

      // value_16_bit_signed_int = conv_bit_size(tmp_16_bit_signed_int, bits_per_sample);
      value_16_bit_signed_int = conv_bit_size(
        tmp_16_bit_unsigned_int,
        bits_per_sample
      );
      ...
      new_32_bit_array[index_32_bit_floats] =
        0 < value_16_bit_signed_int
  ...
  var convert_32_bit_float_into_signed_16_bit_int_lossy = function(
    input_32_bit_buffer
  ) {
    // this method is LOSSY - intended as preliminary step when saving audio into WAV format files
    ...
    for (var index = 0; index < size_source_buffer; index++) {
      value_16_bit_signed_int = ~~(0 < input_32_bit_buffer[index]
  ...
  var show_object = function(
    ...
    limit_size_buffer =
      limit_size_buffer === 0 ? 9999999999 : limit_size_buffer; // no limit if given 0 as limit
    ...
    if ("partial" == mode) {
      ...
      for (property in given_obj) {
        ...
        if (property.substring(0, 3) == "cb_") {
          ...
          var max_value_seen = -9999999,
            min_value_seen = 9999999;
  ...
  var diff_entire_buffers = function(
    left_obj,
    right_obj,
    size_buffer,
    given_spec
  ...
  var diff_buffers = function(left_obj, right_obj, given_spec) {
    ...
    switch (extent) {
      ...
      default: {
        throw new Error(
          "ERROR - failed to find recognized value of spec.extent : ",
          extent
        );
      }
    }
