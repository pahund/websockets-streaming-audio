var ww_client_socket = (function() {
  ...
  var websocket_connection = (function() {
    ...
    if (!("WebSocket" in self)) {
      send_console_to_browser.log(
        "ERROR - boo hoo ... websockets is not available on this browser - use firefox"
      );
      return;
    }
    ...
    send_console_to_browser.log(
      "very cool web sockets are supported by your browser"
    );
    ...
    */
      ...
      if (typeof event.data === "string") {
        ...
        if (typeof received_json.streaming_is_done !== "undefined") {
          if ("yes" == received_json.streaming_is_done) {
            ...
            console.log(
              "OK received_json.streaming_is_done == yes  local_max_index ",
              local_max_index,
              " ... so call cb_stream_is_complete ",
        ...
        console.log(
          "Blob received on client browser side of length data.size ",
          event.data.size
        );
    ...
    return {
        ...
        function wait_for_socket_connection(socket, callback) {
          setTimeout(function() {
            ...
            if (socket.readyState === 1) {
              ...
              console.log(
                "... waiting for web socket connection to come online"
              );
      ...
      close_socket: function() {
        send_console_to_browser.log(
          "NOTICE - about to close socket intentionally"
        );
  ...
  var set_stream_is_complete_cb = function(given_cb_stream_is_complete) {
    // supplied by calling client
    ...
    return function(given_msg) {
      ...
      switch (given_mode) {
        case "mode_stream_audio": {
          //  stream audio buffer from server
