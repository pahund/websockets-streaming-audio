var server_streaming_audio = (function() {
  ...
  var streaming_is_done = function(given_max_index, curr_ws) {
    console.log(
      "TOOOP streaming_is_done  ... given_max_index ",
      given_max_index
    );
    console.log(
      "TOOOP streaming_is_done  ... given_max_index ",
    ...
    console.log(
      "SEND ---------- streaming_is_done_msg ",
      streaming_is_done_msg
    );
    console.log("SEND -------- json DONE --------");

    curr_ws.send(JSON.stringify(streaming_is_done_msg), {
      binary: false,
      mask: false
    }); //  json message
  ...
  var send_client_source_data_info = function(audio_obj, curr_websocket) {
    ...
    curr_websocket.send(JSON.stringify(all_property_tags), {
      binary: false,
      mask: false
    });
  };
  ...
  function stream_file_into_socket(received_json, curr_ws) {
    ...
    var do_stream = function(
      ...
      console.log(
        "about to stream ... requested_input_filename ",
        requested_input_filename
      );
      ...
      fs.stat(requested_input_filename, function(error, stat) {
        ...
        var read_from_stream = function(socket_conn) {
          ...
          while ((curr_buffer = read_stream.read())) {
            var fresh_data_buffer = shared_utils.convert_16_bit_signed_int_to_32_bit_float(
              curr_buffer
            );
            ...
            if (
              fresh_data_buffer.length < temp_stream_chunk_obj.buffer.length
            ) {
          ...
          // stens TODO 20150115
          socket_conn.send(temp_stream_chunk_obj.buffer, {
            binary: true,
            mask: false
          }); // binary buffer
    ...
    var init_stream = function(
      ...
      var requested_input_filename = path.join(
        __dirname,
        media_dir,
        media_filename
      );
      ...
      shared_utils.parse_wav_header(
        wav_input_file_obj,
        requested_input_filename,
        function(error, header_obj) {
          if (error) {
            console.error(error);
            return;
          } else {
  ...
  var file_manager = (function() {
    ...
    return {
      read_file_pop_buffer_stream_back_to_client_async: function(
        received_json,
        curr_ws,
        request_status
      ) {
        if (request_status === request_new) {
          ...
          curr_stream_session.init_stream(
            media_dir,
            media_path,
            received_json,
            curr_ws,
  ...
  var route_msg = function(received_json, curr_ws) {
    ...
    if (typeof requested_action == "undefined") {
      console.error(
        "ERROR - failed to see property : requested_action in client JSON msg"
      );
      // process.exit(8);
    }
    ...
    switch (requested_action) {
      case "stream_audio_to_client": {
        ...
        if (request_status === request_ongoing) {
          if (request_number === previous_request_number) {
            ...
            console.log(
              "cool we have already stopped this stream request so ignoring stream request"
            );
      ...
      default: {
        console.error(
          "ERROR - failed to recognize client requested_action : ",
          requested_action
        );
        // process.exit(8);
